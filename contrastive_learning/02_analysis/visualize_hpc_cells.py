...
def main():
    ...
    dot_radius = 2
    fill_color_hpc = (255, 210, 51, 255)  # Color for HPC tiles
    fill_color_tumor = (0, 255, 255, 255)  # Cyan for tumor cells
    fill_color_immune = (255, 0, 0, 255)  # Red for immune cells
    ...
    for slide_id in top_slides:
        ...
        downsample_factor = float(slide.properties["openslide.mpp-x"]) / target_magnification
        ...
        if "TCGA" in str(slide_id):
            cell_summary_path = glob(os.path.join(args.hover_net_path, str(slide_id) + ".*_files", "cell_summary","*.csv"))[0]
        else:
            cell_summary_path = glob(os.path.join(args.hover_net_path, str(slide_id) + "*_files", "cell_summary","*.csv"))[0]
        ...
        window_size_40x = convert_mags_from_20x(224, float(slide.properties["openslide.mpp-x"]))
        ...
        # Blend the overlay with the downsampled WSI
        downsampled_wsi_overlay= Image.alpha_composite(downsampled_wsi, overlay)

        # Save the overlay image
        downsampled_wsi_overlay.save(os.path.join(output_dir_sub, f"{slide_id}_{str(args.hpc)}.png"))
