...
def parse_arguments():
    ...
    parser.add_argument('--nb_tiles_train', type=int, help="Number of tiles for the train set.")
...
def process_tiles(final_sampled_tiles, wsi_path, output_dir, window_size, normalize = '57,22,-8,20,10,5', tcga_flag=True, htan_flag=False):
    ...
    for slide_name in tqdm(final_sampled_tiles["slide"].unique(), desc="Processing slides"):
        ...
        slide_path = next((i for i in wsi_list if os.path.splitext(os.path.basename(i))[0] == slide_name), None)
        if not slide_path:
            continue
        ...
        final_sampled_tiles_sub = final_sampled_tiles[final_sampled_tiles["slide"] == slide_name]
        for _, row in final_sampled_tiles_sub.iterrows():
            try:
                ...
                tile_outpath = os.path.join(output_dir, f"{row['subset']}_{row['slide']}_{row['CellID']}.jpeg")
                if not os.path.isfile(tile_outpath):
                    tile.save(tile_outpath)
            except Exception as e:
                print(f"Error processing tile {row['CellID']} in {slide_name}: {e}")
...
def main():
    ...
    csv_paths = glob(os.path.join(args.results_dir, "*_files", "cell_summary", "*.csv"))
    all_cells_df = []
